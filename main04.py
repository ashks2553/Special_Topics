...
# 6. نمودارهای تحلیل ویژگی‌ها
plt.figure(figsize=(14, 10))

plt.subplot(2, 2, 1)
sns.boxplot(x='عملکرد_کنکور', y='معدل', data=df, palette='Set2', width=0.6)
plt.title('رابطه معدل با عملکرد کنکور')
plt.xlabel('عملکرد کنکور')
plt.ylabel('معدل')
plt.xticks([0,1,2,3], ['ضعیف', 'متوسط', 'خوب', 'عالی'])
plt.subplot(2, 2, 2)
sns.violinplot(x='عملکرد_کنکور', y='ساعت_مطالعه_روزانه', data=df, palette='Set3', width=0.8)
plt.title('رابطه ساعت مطالعه با عملکرد کنکور')
plt.xlabel('عملکرد کنکور')
plt.ylabel('ساعت مطالعه روزانه')
plt.xticks([0,1,2,3], ['ضعیف', 'متوسط', 'خوب', 'عالی'])

plt.subplot(2, 2, 3)
sns.countplot(x='جنسیت', hue='عملکرد_کنکور', data=df, palette='Paired')
plt.title('توزیع عملکرد بر اساس جنسیت')
plt.xlabel('جنسیت')
plt.ylabel('تعداد')
plt.xticks([0,1], ['مرد', 'زن'])
plt.legend(title='عملکرد', labels=['ضعیف', 'متوسط', 'خوب', 'عالی'])

plt.subplot(2, 2, 4)
sns.barplot(x='عملکرد_کنکور', y='میزان_استرس', data=df, ci=None, palette='coolwarm')
plt.title('رابطه استرس با عملکرد کنکور')
plt.xlabel('عملکرد کنکور')
plt.ylabel('میانگین میزان استرس')
plt.xticks([0,1,2,3], ['ضعیف', 'متوسط', 'خوب', 'عالی'])

plt.tight_layout()
plt.show()

# 7. مقایسه خوشه‌ها با عملکرد واقعی
